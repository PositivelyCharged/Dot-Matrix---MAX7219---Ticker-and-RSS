...
FLOATRATES_GBP = "http://www.floatrates.com/daily/gbp.xml"

BLYNK_BASE = "https://** YOUR BLYNK LEGACY IP ADDRESS HERE:9443/** YOUR BLYNK LEGACY 1.0 API HERE/get"
BLYNK_ENDPOINTS = {
    "Temp": "/V0",
    "Humidity": "/V1",
    "Pressure": "/V2",
...
def fetch_commodities():
    ...
    try:
        params = {"function": "BRENT", "interval": "daily", "apikey": "** YOUR ALPHA VANTAGE API KEY HERE **"}
...
def fetch_gbp_floatrates():
    try:
        logging.info("Fetching GBP FloatRates...")
        r = requests.get(FLOATRATES_GBP, headers=HEADERS, timeout=10)
        r.raise_for_status()
        root = ET.fromstring(r.text)

        rates = {}
        for item in root.findall("item"):
            target = item.findtext("targetCurrency", "").strip().upper()
            rate_text = item.findtext("exchangeRate", "").strip()
            if not target or not rate_text:
                continue
            try:
        ...
        for code in FX_CODES:
            ...
            if code in rates:
                ...
                logging.warning(f"[CACHE] No rate for {code} in FloatRates")
    except HTTPError as e:
        logging.error(f"[CACHE] HTTP error fetching FloatRates: {e}")
    except ConnectionError as e:
        logging.error(f"[CACHE] Connection error fetching FloatRates: {e}")
    except Timeout as e:
        logging.error(f"[CACHE] Timeout error fetching FloatRates: {e}")
    except Exception as e:
        logging.error(f"[CACHE] An unexpected error occurred while fetching FloatRates: {e}")
